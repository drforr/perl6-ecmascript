...
# lexer rules.
  ...
  rule IdentifierPart {
    ( <IdentifierStart> ) => <IdentifierStart> # Avoids ambiguity, as some <Identifier>Start chars also match following alternatives.
  | <UnicodeDigit>
  | <UnicodeConnectorPunctuation>
  }
	
  rule UnicodeLetter {		# Any character in the Unicode categories "Uppercase letter (Lu)", 
    <[\x{0041}..\x{005A}]>	# "Lowercase letter (Ll)", "Titlecase letter (Lt)",
  | <[\x{0061}..\x{007A}]>	# "Modifier letter (Lm)", "Other letter (Lo)", or "Letter number (Nl)".
  | '\x{00AA}'
  | '\x{00B5}'
  | '\x{00BA}'
  | <[\x{00C0}..\x{00D6}]>
  ...
  rule UnicodeCombiningMark {	# Any character in the Unicode categories "Non-spacing mark (Mn)"
    <[\x{0300}..\x{034E}]>	# or "Combining spacing mark (Mc)".
  | <[\x{0360}..\x{0362}]>
  | <[\x{0483}..\x{0486}]>
  | <[\x{0591}..\x{05A1}]>
  | <[\x{05A3}..\x{05B9}]>
  ...
  rule WhiteSpace { # Tab, vertical tab, form feed, space, non-breaking space and any other unicode "space separator".
    ( '\t' | '\v' | '\f' | ' ' | '\x{00A0}' )	# {$channel=HIDDEN;}
  }
